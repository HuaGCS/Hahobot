...
from hahobot.config.schema import DingTalkConfig, DingTalkInstanceConfig
...
class NanobotDingTalkHandler(CallbackHandler):
    ...
    Parses incoming messages and forwards them to the Hahobot channel.
    """
    ...
    async def process(self, message: CallbackMessage):
        ...
        try:
            ...
            # Forward to Hahobot via _on_message (non-blocking).
...
class DingTalkChannel(BaseChannel):
    ...
    async def _send_markdown_text(self, token: str, chat_id: str, content: str) -> bool:
        return await self._send_batch_message(
            ...
            chat_id,
            "sampleMarkdown",
            {"text": content, "title": "Hahobot Reply"},
        )
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        if msg.content and msg.content.strip():
            delivered = await self._send_markdown_text(token, msg.chat_id, msg.content.strip())
            if not delivered:
        ...
        for media_ref in msg.media or []:
            ...
            filename = self._guess_filename(media_ref, self._guess_upload_type(media_ref))
            fallback_delivered = await self._send_markdown_text(
                token,
                msg.chat_id,
                f"[Attachment send failed: {filename}]",
            )
            if not fallback_delivered:
                raise RuntimeError("DingTalk attachment failure fallback was not delivered")
