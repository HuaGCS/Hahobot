"""WebSocket server channel: hahobot acts as a WebSocket server for connected clients."""
...
from hahobot.config.schema import Base
...
def _issue_route_secret_matches(headers: Any, configured_secret: str) -> bool:
    ...
    header_token = headers.get("X-Hahobot-Auth") or headers.get("x-hahobot-auth")
    if not header_token:
...
class WebSocketChannel(BaseChannel):
    """Run a local WebSocket server; forward text/JSON messages to the bus."""
    ...
    def _handle_token_issue_http(self, connection: Any, request: Any) -> Any:
        ...
        token_value = f"hbwt_{secrets.token_urlsafe(32)}"
