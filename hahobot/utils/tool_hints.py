...
from hahobot.utils.path import abbreviate_path
...
def format_tool_hints(tool_calls: list, max_length: int = 40) -> str:
    ...
    for tc in tool_calls:
        ...
        if not isinstance(name, str) or not name:
            # Degenerate/malformed tool call (e.g. a model emits name=None); skip it
            # instead of raising AttributeError on the whole turn. nanobot 8248d075.
            continue
    ...
    return ", ".join(f"{hint} \u00d7 {count}" if count > 1 else hint for hint, count in hints)
