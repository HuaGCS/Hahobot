...
_TOOL_RESULTS_DIR = ".hahobot/tool-results"
...
def split_message(content: str, max_len: int = 2000) -> list[str]:
    ...
    if not content:
        return []
    # A non-positive max_len cannot advance the cut pointer and would loop forever.
    if max_len <= 0:
        return [content]
...
def build_status_content(
    *,
    ...
    Args:
        memory_layers_text: Optional pre-formatted persona memory-layer summary.
    ...
    if memory_layers_text:
        lines.append(memory_layers_text)
...
def sync_workspace_templates(workspace: Path, silent: bool = False) -> list[str]:
    ...
    try:
        tpl = pkg_files("hahobot") / "templates"
    except Exception:
        return []
    if not tpl.is_dir():
        return []
