...
def _resolve_local_schema_ref(root: dict[str, Any], ref: str) -> Any:
    """Resolve a URI-decoded local JSON Pointer without accepting remote refs."""
    ...
    def rewrite(value: Any) -> Any:
        ...
        if isinstance(ref, str) and not ref.startswith("#/$defs/"):
            try:
                ...
                is_rewritable_ref = ref.startswith("#") and (not pointer or pointer.startswith("/"))
            ...
            if name is None:
                try:
                    ...
                    while isinstance(existing_defs, dict) and name in existing_defs:
                        name += "_"
                    # Reserve before descending so recursive refs terminate.
...
def _normalize_nullable_schema(schema: dict[str, Any]) -> dict[str, Any]:
    """Normalize nullable forms in structural subschemas."""
...
def _normalize_schema_for_openai(schema: Any) -> dict[str, Any]:
    """Normalize MCP JSON Schema patterns for provider tool definitions."""
