...
class AnthropicProvider(LLMProvider):
            ...
            for item in content:
                if isinstance(item, dict):
                    if item.get("type"):
                        ...
                        # text block instead of emitting one the API rejects.
                        blocks.append(
                            {
                                "type": "text",
    ...
    def _convert_user_content(content: Any) -> Any:
        ...
        for item in content:
            ...
            if not item.get("type"):
                # A tool that returned a bare dict (or a list of dicts) lands here;
                # coerce it to a text block instead of emitting a block the API
                # rejects with "content.0.type: Field required".
                result.append(
                    {
                        "type": "text",
                        "text": AnthropicProvider._stringify_typeless_block(item),
    ...
    def _build_kwargs(
        ...
        omit_temperature = adaptive_only or model_name.lower() in _SAMPLING_DEPRECATED_MODELS
