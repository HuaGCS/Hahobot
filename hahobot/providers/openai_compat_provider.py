...
if TYPE_CHECKING:
    from hahobot.providers.registry import ProviderSpec
...
def _uses_openrouter_attribution(spec: ProviderSpec | None, api_base: str | None) -> bool:
    """Apply Hahobot attribution headers to OpenRouter requests by default."""
...
class OpenAICompatProvider(LLMProvider):
    ...
    def _sanitize_messages(self, messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        def unique_tool_id(value: Any, used_ids: set[str], idx: int) -> str:
            """Map *value* to a normalized id that is unique within one message.

            Some providers reuse the same tool_call id across parallel calls;
            allowing duplicates here makes the paired tool result ambiguous.
            """
        ...
        def map_tool_result_id(value: Any) -> Any:
            """Resolve a tool result id, consuming the FIFO of deduped ids."""
            if not isinstance(value, str):
                return value
    ...
    def _parse_chunks(cls, chunks: list[Any]) -> LLMResponse:
        ...
        # the response so downstream tool messages do not collide.
        _seen_tc_ids: set[str] = set()
    ...
    async def chat_stream(
        ...
        idle_timeout_s = LLMProvider._stream_idle_timeout_s()
        try:
            if self._should_use_responses_api(model, reasoning_effort):
                try:
                    body = self._build_responses_body(
