...
def _unique_item_id(item_id: str, used: set[str]) -> str:
    ...
    Codex rejects resumed conversations that carry duplicate ``rs_*`` /
    ``msg_*`` / ``fc_*`` item ids, so the converter must rename collisions
    while keeping ``call_id`` (used for tool-result linkage) untouched.
    """
