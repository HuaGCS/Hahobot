...
TOKEN_APP_NAME = "hahobot"
USER_AGENT = "hahobot/0.1"
...
class GitHubCopilotProvider(OpenAICompatProvider):
    ...
    async def _get_copilot_access_token(self) -> str:
        ...
        # Guard the check-then-fetch against concurrent chat() calls: after the
        # token expires, two coroutines would otherwise both exchange a new token
        # and clobber each other. Double-checked locking keeps it to one fetch per
        # expiry window. Ported from nanobot `28011413`.
        async with self._copilot_token_lock:
            ...
            if not github_token or not github_token.access:
                raise RuntimeError(
                    "GitHub Copilot is not logged in. Run: hahobot provider login github-copilot"
                )
