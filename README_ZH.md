...
对于 HTTP 类传输（`sse` / `streamableHttp`），你配置的 MCP 服务器 URL 是受信任的——
本地服务器（如 `http://127.0.0.1:3211/mcp`）照常可用。作为纵深防御：
若某个（公网）服务器在请求过程中把你*重定向*到另一个内网 / 私有地址，该重定向跳转会被拒绝，
这样远端服务器就无法借此把 hahobot 引导去访问你的内网。
...
- `tag` 由 LLM 提议(`preference` / `project` / `reference` / `feedback` / `user` / `experience`),被服务端规范化,未知值降级为 `preference`。`experience` 专门用来记录"这类任务的成功模式"(工具序列、踩坑经验、规避方案),区别于一般事实。
