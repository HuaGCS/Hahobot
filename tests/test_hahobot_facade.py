"""Tests for the Hahobot programmatic facade."""
...
def test_from_config_missing_file():
    with pytest.raises(FileNotFoundError):
        Hahobot.from_config("/nonexistent/config.json")
...
def test_from_config_default_path():
    ...
    with patch("hahobot.config.loader.load_config") as mock_load, \
         patch("hahobot.hahobot._make_provider") as mock_prov:
...
def test_workspace_override(tmp_path):
    ...
    bot = Hahobot.from_config(config_path, workspace=custom_ws)
    assert bot._loop.workspace == custom_ws


def test_sdk_make_provider_uses_github_copilot_backend():
    from hahobot.config.schema import Config
    from hahobot.hahobot import _make_provider
