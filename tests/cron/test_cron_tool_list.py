...
def test_add_at_job_uses_default_timezone_for_naive_datetime(tmp_path) -> None:
    tool = _make_tool_with_tz(tmp_path, "Asia/Shanghai")
    tool.set_context("telegram", "chat-1")
    naive = (datetime.now(UTC) + timedelta(days=1)).replace(tzinfo=None, microsecond=0)
...
@pytest.mark.parametrize(
    ...
    [
        (60, "0 8 * * *", None),
        (60, None, "2099-01-01T00:00:00+00:00"),
        (None, "0 8 * * *", "2099-01-01T00:00:00+00:00"),
        (None, None, None),
    ],
)
def test_add_job_requires_exactly_one_schedule_field(
    tmp_path,
    ...
    result = tool._add_job(None, "Reminder", every_seconds, cron_expr, None, at)

    assert result == "Error: exactly one of every_seconds, cron_expr, or at is required"
    assert tool._cron.list_jobs() == []


def test_add_job_rejects_past_one_time_schedule(tmp_path) -> None:
    tool = _make_tool(tmp_path)
    tool.set_context("telegram", "chat-1")

    result = tool._add_job(None, "Too late", None, None, None, "2000-01-01T00:00:00+00:00")

    assert "is not in the future" in result
    assert tool._cron.list_jobs() == []
