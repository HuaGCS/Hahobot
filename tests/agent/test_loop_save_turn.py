...
def _make_full_loop(tmp_path: Path) -> AgentLoop:
    ...
    loop._ensure_background_token_consolidation = MagicMock()  # type: ignore[method-assign]
    return loop
...
async def test_process_system_subagent_persists_followup_without_prompt_duplication(
    ...
    followups = [
        message for message in session.messages if message.get("subagent_task_id") == "task-1"
    ]
