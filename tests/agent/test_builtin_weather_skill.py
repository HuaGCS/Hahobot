...
def test_weather_skill_uses_windows_safe_single_today_request() -> None:
    ...
    assert "On Windows PowerShell, use `curl.exe`" in content
    assert "bare `curl` may resolve to `Invoke-WebRequest`" in content
    assert 'curl -s "https://wttr.in/London?1&m"' in content
    assert 'curl.exe -s "https://wttr.in/Berlin.png" -o weather.png' in content
    assert "/tmp/weather.png" not in content
    assert (
        "Do not fetch current conditions separately when a today or forecast request already "
        "includes them." in normalized
    )
