...
from hahobot.channels.matrix import _build_matrix_text_content

import hahobot.channels.matrix as matrix_module
from hahobot.bus.events import OutboundMessage
from hahobot.bus.queue import MessageBus
from hahobot.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from hahobot.config.schema import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("hahobot.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "hahobot.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("hahobot.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "hahobot.channels.matrix.asyncio.create_task", _fake_create_task
    )
