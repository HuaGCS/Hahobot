"""Tests for the WebSocket server channel."""
...
from hahobot.channels.websocket import (
    WebSocketChannel,
    WebSocketConfig,
...
_LOOPBACK_REQUIRED = pytest.mark.skipif(
    ...
    reason="loopback sockets are not permitted in this sandbox",
)
...
def test_issue_route_secret_matches_bearer_and_header() -> None:
    ...
    x_headers = Headers([("X-Hahobot-Auth", "my-secret")])
...
@pytest.mark.asyncio
async def test_registry_discovers_websocket_channel() -> None:
    from hahobot.channels.registry import load_channel_class
...
async def test_end_to_end_server_pushes_streaming_deltas_to_client(bus: MagicMock) -> None:
    ...
    try:
        async with websockets.connect(f"ws://127.0.0.1:{port}/ws?client_id=stream-tester") as client:
            ...
            assert delta1 == {"event": "delta", "text": "Hello ", "stream_id": "s1"}
            assert delta2 == {"event": "delta", "text": "world", "stream_id": "s1"}
            assert end == {"event": "stream_end", "stream_id": "s1"}
    finally:
        await channel.stop()
        await asyncio.wait_for(server_task, timeout=5.0)
