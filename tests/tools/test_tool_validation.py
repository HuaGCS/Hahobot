...
def test_exec_extract_absolute_paths_captures_quoted_paths() -> None:
    cmd = 'cat "/tmp/data.txt" "~/.hahobot/config.json"'
    paths = ExecTool._extract_absolute_paths(cmd)
    assert "/tmp/data.txt" in paths
    assert "~/.hahobot/config.json" in paths
...
def test_exec_guard_allows_media_path_outside_workspace(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("hahobot.agent.tools.shell.get_media_dir", lambda: media_dir)
...
def test_exec_guard_blocks_windows_drive_root_outside_workspace(monkeypatch) -> None:
    import hahobot.agent.tools.shell as shell_mod
...
def test_validate_params_rejects_nested_non_finite_numbers() -> None:
    tool = CastTestTool(
        {
            "type": "object",
            "properties": {
    ...
    errors = tool.validate_params({"samples": [{"value": float("inf")}]})

    assert errors == ["samples[0].value must be finite"]
