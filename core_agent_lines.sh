...
echo "hahobot line count"
echo "=================="
echo ""

echo "Core runtime"
echo "------------"
core_agent=$(count_top_level_py_lines "hahobot/agent")
core_bus=$(count_top_level_py_lines "hahobot/bus")
core_config=$(count_top_level_py_lines "hahobot/config")
core_cron=$(count_top_level_py_lines "hahobot/cron")
core_heartbeat=$(count_top_level_py_lines "hahobot/heartbeat")
core_session=$(count_top_level_py_lines "hahobot/session")
...
echo "----------------"
extra_tools=$(count_recursive_py_lines "hahobot/agent/tools")
extra_skills=$(count_skill_lines "hahobot/skills")
extra_api=$(count_recursive_py_lines "hahobot/api")
extra_cli=$(count_recursive_py_lines "hahobot/cli")
extra_channels=$(count_recursive_py_lines "hahobot/channels")
extra_utils=$(count_recursive_py_lines "hahobot/utils")
...
echo "Notes"
echo "-----"
echo "  - agent/ only counts top-level Python files under hahobot/agent"
echo "  - tools/ is counted separately from hahobot/agent/tools"
echo "  - skills/ counts .md, .py, and .sh files"
echo "  - not included here: command/, providers/, security/, templates/, hahobot.py, root files"
