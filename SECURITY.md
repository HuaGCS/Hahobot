...
- Config files and API keys (`~/.hahobot/config.json`) → **hidden** (masked by tmpfs)
...
- Keep authentication data in `~/.hahobot/whatsapp-auth` secure (mode 0700)
...
- Subscribe to security advisories for hahobot and its dependencies
...
2. **Use a Dedicated User**
   ```bash
   sudo useradd -m -s /bin/bash hahobot
   sudo -u hahobot hahobot gateway
   ```
...
3. **Set Proper Permissions**
   ...
   chmod 700 ~/.hahobot
   chmod 600 ~/.hahobot/config.json
   chmod 700 ~/.hahobot/whatsapp-auth
   ```

4. **Enable Logging**
   ```bash
   # Configure log monitoring
   tail -f ~/.hahobot/logs/hahobot.log
   ```
...
- **Chat history is stored locally** - protect the `~/.hahobot` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.hahobot/logs/hahobot.log
   ```
