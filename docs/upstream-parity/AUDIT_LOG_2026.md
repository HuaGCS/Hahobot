# Upstream Parity Audit Log — 2026

> Historical archive. For the current audit boundary and actionable parity state, start with
> [`UPSTREAM_PARITY.md`](../../UPSTREAM_PARITY.md). This file preserves the detailed 2026 audit
> record so rationale does not live only in Git history.

This archive is intentionally append-oriented. Older entries describe the state at the date shown;
when a later audit supersedes an earlier conclusion, the current root ledger is authoritative.
In particular, older statements about Hahobot having no WebUI, AtomGit being web-fetch-only, or a
future source needing re-fetch may have been superseded by later implementation and clone-based
audits. They remain here as dated evidence, not current requirements.
...
- `nanobot` (`2026-08-13` follow-up adaptation; audit boundary unchanged at
  ...
  `tests/test_skill_commands.py` pass; release validation outside the restricted local-socket
  sandbox completed with 2,400 passed and four environment skips. MCP runtime failure visibility
...
- `nanobot` / `GenericAgent` (`2026-08-12`): fetched without tags. Nanobot advanced linearly by 19
  ...
  array/object paths (`hahobot/agent/tools/base.py`, `tests/tools/test_tool_validation.py`). Follow-up
  ported Matrix thread session isolation (`057e8f7af`): inbound text and media events now pass
  `matrix[/instance]:<room>:thread:<root-event>` as the session override while retaining the room id
...
- `nanobot` (`2026-07-27` pass): re-checked upstream `main` through `cf1e801a`
  ...
  `stat()` before loading (`cdb2df49`); config/admin writes use mode-preserving atomic replace
  (`28102382` / `b2cf37da`); provider-bound message values recursively sanitize malformed UTF-16
  surrogates (`89d8c055`); Qwen model families receive their model-level thinking wire format
  (`79d94553`); Gemini image requests follow the final model-specific aspect-ratio/image-size matrix
  (`4986590b`..`cf1e801a`). Persistence now accepts string cron timestamps and skips null run-history,
  session metadata, history rows, and skill requirement collections (`b81c0558` / `299bcf49` /
  `7c94ba96` / `745757cc` / `259d8a01`). Slack/Feishu preserve fenced Markdown tables and Feishu
  tolerates null post/card fields (`5851bd43` / `81951817` / `fb881543` / `aaf2eef5`); exec guards
  detect absolute paths after `=` (`78f4c132`); Telegram fence splitting now guarantees forward
  progress even for a minified one-line fence or tiny split budget (`017a4946` / `98d66177` /
  `7e9426d9`); MCP schemas hoist arbitrary URI-decoded local JSON Pointer refs into `$defs` without
  breaking recursion (`9aae7485` / `c1899e2c`); Dream advances its cursor only after a completed
  phase-2 batch (`4e2640f2` / `15e42059`). The full length-recovery chain
  (`b19039f9`..`b55b76d7`) was adapted so truncated segments survive into the final answer and stay
  one visible streamed message across channels. Reviewed but not copied: config watcher/env-ref
  work is already covered by Hahobot's runtime reload plus startup config resolution; pairing,
  triggers, the React/native WebUI, and broad new channel/provider surfaces do not map directly.
  DingTalk DM gating/sender labels and broader provider/channel additions remain demand-driven
  watchlist items.
- `GenericAgent` (`2026-07-27` pass): upstream `main` was force-rewritten; audited the new mainline
  by date/first-parent through `5c3fc72d` (`2026-07-25`) rather than pretending the previous
  `d69ec880` is an ancestor. Its empty-text-block fix is already covered by Hahobot's shared
  `_sanitize_empty_content` provider normalization. Desktop/Tauri packaging and conductor/runtime
  structure remain architecture-specific; no direct port this pass.
- `claude-mem` (`2026-07-27` pass): temporary-clone audit through `132b4634`
  (`2026-07-23`); 183 commits since `f5633c1f`. The range is dominated by hosted SyncHub/worker,
  Chroma, plugin, and release work, which does not fit Hahobot's local file-first memory source of
  truth. Explicit `filesRead` / `filesModified` evidence on archive observations is a useful recall
  idea and remains a focused watchlist item; it needs a local sidecar migration/query contract,
  not an incidental schema addition.
- `nocturne_memory` (`2026-07-27` pass): still `2cbfb8a` (`2026-07-21`), with no commits since the
  previous audit boundary; the patch-only/file-first decisions are unchanged.
- `jiuwenswarm` (`2026-07-27` pass): re-checked `develop@de623dd9` (`2026-07-26`); 116 commits since
  `caec89ca`. Two ideas map cleanly onto existing Hahobot surfaces and were adopted: the cron store
  now locks the complete cross-process read-modify-write transaction and commits by atomic replace
  (`1d5c54bdf`, adapted); async callers wait in a dedicated store worker pool, execution claims are
  distinct from replaceable timer sleepers, cancellation is linearized with atomic replace, and
  workspace rebind drains old automated ticks before publishing the new store. Server-rendered
  WebUI history restores persisted media after refresh through Hahobot's existing traversal-safe
  `/app/media` mapping (`94310a3ad`, adapted). Broader
  cross-process config transactions, secret redaction, Team/cluster authority, terminal/TUI, and
  jiuwenbox changes remain watchlist or architectural divergences rather than partial ports.
...
- `nanobot` (`2026-07-01` pass): re-checked against upstream `main` through `c78421cf`
  ...
  guard).
  Historical note superseded by the 2026-07-27 audit: `f6d1dba3` (tolerate `EINVAL` on parent-dir
  fsync in cron `_atomic_write`) was not applicable to the older memory-only atomic helper, but Cron
  and config/admin writes now use `utils.helpers._write_text_atomic`, which fsyncs the parent and
  tolerates platform `OSError` / `NotImplementedError` failures there. Other larger deltas reviewed
  and **not ported** in that pass: `070aed8a` (skip non-file-edit tools in `apply_final_call_ids`) is **not
...
| Session persistence durability | `synced` | Session full rewrites use atomic replace, corrupt JSONL can be repaired for load/list flows, recovered sessions are forced through the next clean rewrite, and a persisted `metadata: null` is normalized to `{}` instead of crashing session construction. |
...
| Provider thinking toggles | `synced` | `ProviderSpec.thinking_style` owns provider-wide DashScope, DeepSeek, VolcEngine/BytePlus, and MiniMax wire formats; `_MODEL_THINKING_STYLES` adds model-level Qwen `enable_thinking` through any compatible endpoint. Legacy assistant turns receive empty `reasoning_content` when thinking mode is enabled mid-session, while model-scoped Qwen matching leaves unrelated models on the same endpoint unchanged. |
...
| Exec assignment-form path guard | `synced` | Workspace restriction extracts absolute paths after `=` as well as whitespace (for example `--output=/tmp/x`), while treating `=~` in regex/query syntax as an operator rather than a home path. Adapted from nanobot `78f4c132`. |
...
| Malformed history-entry guard | `synced` | `MemoryStore._read_entries` drops `history.jsonl` values that are not objects or carry a malformed shape (non-int cursor, non-string timestamp/content), warning once. This covers both syntax errors and valid JSON scalars/null, so external or older writers cannot crash Dream/consolidation. Ported from nanobot `f85101f0` / `745757cc`; tests in `tests/agent/test_memory_store.py`. |
...
| Length-truncated response recovery | `synced` | Consecutive `finish_reason="length"` segments are anchored by a short delivered tail, accumulated into the final answer, and emitted as one logical streamed message. Recovery state resets when tool work begins; missing terminal deltas and max-iteration closure are delivered once without repeating the visible prefix. Adapted from nanobot `b19039f9`..`b55b76d7`; tests cover runner, dispatch stream ids, and channel-manager coalescing. |
...
| Dream batch retryability | `synced` | Dream advances its archive cursor and compacts processed history only when phase 2 completed, including a valid no-op completion. Failed or max-iteration batches retain their cursor and source entries for the next run instead of silently dropping memory work. Ported from nanobot `4e2640f2` / `15e42059`. |
...
| Provider request sanitization | `synced` | Role alternation repair recovers a trailing assistant message as `user` when otherwise only `system` content would remain, multimodal normalization drops whitespace-only text blocks, and the shared provider boundary recursively replaces lone UTF-16 surrogate code points in nested string values before JSON encoding. |
...
| Skill filtering / idle compact / MCP tool filtering | `synced` | `agents.defaults.disabledSkills`, `agents.defaults.idleCompactAfterMinutes` (plus `sessionTtlMinutes` alias), and `tools.mcpServers.<name>.enabledTools` are wired through local runtime, tests, and docs. Skill metadata loading treats null/wrong-shaped `requires.bins` / `requires.env` as empty instead of crashing discovery. |
...
| MCP local JSON Pointer schemas | `synced` | OpenAI-facing MCP schemas preserve existing `#/$defs/...` entries and hoist arbitrary local refs such as `#/components/schemas/...` into `$defs`, URI-decode pointer tokens, and resolve recursive schemas without infinite traversal. Adapted from nanobot `9aae7485` / `c1899e2c`. |
| Config write durability | `synced` | Main config and admin saves write a sibling temporary file, flush/fsync, preserve an existing POSIX mode, and atomically replace the target; interrupted saves cannot truncate the active JSON file. Ported from nanobot `28102382` / `b2cf37da`. |
| Cron state / scheduler behavior | `synced` | Cron preserves last-run status plus merged run history, accepts camel/snake-case numeric timestamps encoded as integers or strings, and skips null history elements. Every mutation locks a companion file for the complete fresh read-modify-write transaction and commits by atomic replace, so concurrent gateway/CLI processes do not lose each other's jobs; malformed stores abort mutations without being overwritten, and external replacement detection uses `(mtime_ns, inode, size)`. Due/manual runs persist an expiring token claim before external work; only the matching outcome clears it and append-merges the new run record, preventing simultaneous execution and detached snapshots from overwriting fresh history. Async entry points pin the submission store and perform lock/fsync work in a dedicated worker pool, while direct event-loop mutations remain nonblocking. Cancellation is linearized with replace: pre-replace cancellation leaves no commit, while replace-in-progress returns its successful result. Timer sleepers and active executions are tracked separately so ordinary re-arming cannot cancel an in-flight side effect; workspace rebind additionally drains old automated ticks before publishing the initialized target and successful publication wins over cancellation so runtime reload continues. A completed callback whose outcome commit fails retains its lease instead of immediately replaying, but delivery is explicitly at-least-once across ambiguous crash/network boundaries. The scheduler still periodically reloads external edits via `gateway.cron.maxSleepMs`. Nanobot `b81c0558` / `299bcf49`; lock design adapted from jiuwenswarm `1d5c54bdf`. |
...
| Telegram fenced-code-block split | `synced` | Outbound Telegram markdown is split through `_split_telegram_markdown(content, max_len)` at both the `send()` and stream-end edit paths, re-balancing fences across chunks. Hard-cut fallbacks guarantee the cursor advances for minified single-line fences, a cut landing on the opener prefix, leading spaces, and budgets too small to fit a synthetic closer. The generic `split_message` stays shared with Discord. Ported from nanobot `131446fa` / `a5a816ab` / `ffae1dca` plus `017a4946` / `98d66177` / `7e9426d9`. |
...
| Slack/Feishu fenced tables and null payloads | `synced` | Slack and Feishu protect fenced code before table conversion so pipe-delimited code is not rendered as a table. Feishu post/card extraction also treats null text/list/`multi_url` fields as empty instead of raising. Ported from nanobot `5851bd43` / `81951817` / `fb881543` / `aaf2eef5`. |
...
| Document read support | `synced` | `read_file` extracts text from `.docx`, `.xlsx`, and `.pptx` through a small local OOXML parser, while a 100 MiB `stat()` preflight rejects oversized inputs before any text/image/document loader allocates their contents. Adapted from nanobot `cdb2df49`. |
...
| Browser WebUI architecture | `intentional_divergence` | Hahobot ships chat at `/app` inside the existing aiohttp/Jinja gateway rather than adopting nanobot's separate React/Vite SPA. Its server-rendered initial history now restores persisted local/remote media (including media-only messages) with the same traversal-safe URL mapping as live WebSocket frames, adapted from jiuwenswarm `94310a3ad`. |
...
| Gemini image-generation requests | `synced` | Local `tools.imageGen` already supports Gemini; request hints now follow the final model capability matrix so supported image models receive `responseFormat.image.aspectRatio`, and only the models that accept it receive `imageSize`. Adapted from nanobot `4986590b`..`cf1e801a`. StepFun/MiniMax and other speculative backend breadth remain watchlisted until config/admin/docs and delivery tests are added together. |
...
## Historical Update Checklist

This checklist is retained with the old ledger for context. New audits must follow the
[`Update Protocol`](../../UPSTREAM_PARITY.md#update-protocol): update the concise root boundary,
the single source-specific canonical note, and this dated archive without copying the whole result
back into every section.

When syncing from upstream `nanobot`, or when recording a meaningful `GenericAgent`
adoption/divergence decision, preserve these evidence requirements:
